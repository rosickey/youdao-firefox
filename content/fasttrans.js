var fasttransk = {
	...
	prefs: Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService).getBranch("extensions.fasttrans."),
	lfrom: null,
	lto: null,
	resto: null,
	resfrom: null,
	...
	sfrombundle: function(str) {
		return Components.classes["@mozilla.org/intl/stringbundle;1"].getService(Components.interfaces.nsIStringBundleService).createBundle("chrome://fasttrans/locale/fasttrans.properties").GetStringFromName(str);
	},	
	
	openprefs: function() {
		window.openDialog("chrome://fasttrans/content/fasttransprefs.xul","","centerscreen");
	},
	...
	openresult: function(mode) {
		if((fasttransk.selected=='')&&(mode==1)){
			...
			window.openDialog("chrome://fasttrans/content/fasttransresult.xul","fasttrans","centerscreen,dialog=no");
		}
	}, 
